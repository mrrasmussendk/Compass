...
public class NexusGovernedSelectionStrategyTests
{
    private static NexusGovernedSelectionStrategy CreateStrategy(AttributeMetadataProvider? provider = null)
    {
        var store = new InMemoryStore();
        var metaProvider = provider ?? new AttributeMetadataProvider();
        return new NexusGovernedSelectionStrategy(store, metaProvider);
    }
